...
//make sure to make changes for the phone and google oauth
...
app.get('/auth/google/callback', 
    ...
    (req, res) => {
      ...
      const authorizedEmails = ["ritulrashi6@gmail.com"];
      ...
      if (authorizedEmails.includes(userEmail)) {
        ...
        res.send(`
            <script>
                alert("You are not a member and can't login.");
...
app.post('/add-event', ensureAuthenticated, upload.array('photos', 50), (req, res) => {
    const { eventName, eventDate, eventHost, anchor, tracks, sheetMngt, batteriesBy, numAttendees, attendeesSongs, eventDetails } = req.body;

    if (!eventName || !eventDate || !eventHost || !anchor || !tracks || !sheetMngt || !batteriesBy || !numAttendees || !attendeesSongs || !eventDetails) {
...
Sheet Management= ${sheetMngt}
Batteries By= ${batteriesBy}
...
app.post('/edit-event', ensureAuthenticated, (req, res) => {
    ...
    fs.readFile(eventFilePath, 'utf8', (err, data) => {
        ...
        const eventDetails = {};
        let currentKey = ''; // Track the current key for multiline values
        lines.forEach(line => {
            if (line.includes('=')) {
                // This line starts a new key-value pair
                const [key, ...value] = line.split('=');
                currentKey = key.trim();
                eventDetails[currentKey] = value.join('=').trim();
                console.log(`key:${key}, value:${value}`);
            } else if (currentKey && line.trim()) {
                // This line is a continuation of the current key's value
                eventDetails[currentKey] += `\n${line.trim()}`;
                console.log(`key:${currentKey}, value:${line}`);
            }
        });

        console.log("Event Details:", eventDetails['Attendees and their Songs']);
...
app.post('/update-event', ensureAuthenticated, (req, res) => {
    const { eventName, eventDate, eventHost, anchor, tracks, sheetMngt, batteriesBy, numAttendees, attendeesSongs, eventDetails, originalFile } = req.body;
